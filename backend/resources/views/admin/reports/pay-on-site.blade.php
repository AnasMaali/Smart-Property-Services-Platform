...
@section('title', 'Pay-on-Site Report — BLUE Admin')
@section('page-title', 'Pay-on-Site Report')

@section('content')

<div data-report-page="pay-on-site" class="space-y-6">

    <div class="print-only mb-4">
        <h2 class="text-lg font-bold">BLUE — Pay-on-Site Report</h2>
        <p data-print-range class="text-sm text-slate-600"></p>
    </div>

    <div class="no-print">
        <h2 class="text-lg font-semibold text-slate-950">Pay-on-Site Report</h2>
        <p class="mt-1 text-sm text-slate-500">Pending cash never counts as revenue — Pending amount always reflects the current outstanding balance</p>
    </div>
    ...
    <div data-report-empty class="hidden rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">No Pay-on-Site settlements match these filters.</div>
    ...
    <div data-report-content style="display: none;" class="flex flex-col gap-6">
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p data-field="collected_amount" class="mt-2 text-2xl font-semibold text-emerald-600"></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5">
                <p class="text-xs font-medium uppercase tracking-wide text-slate-500">Collected Count</p>
                <p data-field="collected_count" class="mt-2 text-2xl font-semibold text-slate-950"></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5">
                <p class="text-xs font-medium uppercase tracking-wide text-slate-500">Pending (current)</p>
                <p data-field="pending_amount" class="mt-2 text-2xl font-semibold text-amber-600"></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5">
                <p class="text-xs font-medium uppercase tracking-wide text-slate-500">Pending Count (current)</p>
                <p data-field="pending_count" class="mt-2 text-2xl font-semibold text-slate-950"></p>
            </div>
        </div>
            ...
            <table class="w-full min-w-[900px] text-left text-sm">
                <thead>
                    <tr class="border-b border-slate-200 text-xs font-medium uppercase tracking-wide text-slate-500">
                        ...
                        <th class="px-5 py-3">Amount Collected</th>
                        <th class="px-5 py-3">Status</th>
                        <th class="px-5 py-3">Collected At</th>
