/**
 * Admin Reports - Pay-on-Site Report. Reads GET /v1/admin/reports/pay-on-site
 * (App\Actions\Admin\Reports\AdminPayOnSiteReportAction) - `pending_amount`/
 * `pending_count` are always the current outstanding balance, never scoped
 * to the selected range, matching the Financial Dashboard's own behavior.
 */
...
const page = document.querySelector('[data-report-page="pay-on-site"]');
...
if (page) {
    ...
    function renderSummary(summary) {
        page.querySelector('[data-field="collected_amount"]').textContent = formatMoney(summary.collected_amount, summary.currency);
        page.querySelector('[data-field="collected_count"]').textContent = summary.collected_count;
        page.querySelector('[data-field="pending_amount"]').textContent = formatMoney(summary.pending_amount, summary.currency);
    ...
    async function load(params) {
        ...
        try {
            const response = await request(`/api/v1/admin/reports/pay-on-site?${params.toString()}`);
    ...
    function navigate(params) {
        ...
        window.history.pushState({}, '', query ? `/admin/reports/pay-on-site?${query}` : '/admin/reports/pay-on-site');
        load(params);
    }
    ...
    page.querySelector('[data-export-csv]').addEventListener('click', async (event) => {
        ...
        try {
            await downloadReportFile(`/api/v1/admin/reports/pay-on-site/csv?${paramsFromForm(filterForm, FIELDS).toString()}`, 'pay-on-site-report.csv');
        } catch (error) {
    ...
    page.querySelector('[data-export-pdf]').addEventListener('click', async (event) => {
        ...
        try {
            await downloadReportFile(`/api/v1/admin/reports/pay-on-site/pdf?${paramsFromForm(filterForm, FIELDS).toString()}`, 'pay-on-site-report.pdf');
        } catch (error) {
            showError(error.message);
        } finally {
            button.disabled = false;
