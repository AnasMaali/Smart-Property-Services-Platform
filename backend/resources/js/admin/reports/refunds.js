/**
 * Admin Reports - Refund Report. Reads GET /v1/admin/reports/refunds
 * (App\Actions\Admin\Reports\AdminRefundReportAction) - `confirmed_total`/
 * `confirmed_count` are the exact same numbers the Financial Dashboard's
 * `refunds`/`bookings.refunded_count` already show for the same range.
 */
...
const page = document.querySelector('[data-report-page="refunds"]');

if (page) {
    const FIELDS = ['range', 'from', 'to', 'status'];
    ...
    function renderRow(row) {
        ...
        tr.innerHTML = `
            <td class="px-5 py-3.5 text-slate-500">${formatDateTime(row.requested_at)}</td>
            <td class="px-5 py-3.5 font-medium text-slate-900">${escapeHtml(row.booking_number) || '—'}</td>
            <td class="px-5 py-3.5 text-slate-500">${escapeHtml(row.original_payment_reference) || '—'}</td>
            ...
            <td class="px-5 py-3.5 text-slate-500">${escapeHtml(row.reason) || '—'}</td>
        `;
    ...
    function renderSummary(summary) {
        page.querySelector('[data-field="confirmed_count"]').textContent = summary.confirmed_count;
        page.querySelector('[data-field="confirmed_total"]').textContent = formatMoney(summary.confirmed_total, summary.currency);
    ...
    async function load(params) {
        ...
        try {
            const response = await request(`/api/v1/admin/reports/refunds?${params.toString()}`);
            const rows = response.data.refunds || [];
    ...
    function navigate(params) {
        ...
        window.history.pushState({}, '', query ? `/admin/reports/refunds?${query}` : '/admin/reports/refunds');
        load(params);
    }
    ...
    page.querySelector('[data-export-csv]').addEventListener('click', async (event) => {
        ...
        try {
            await downloadReportFile(`/api/v1/admin/reports/refunds/csv?${paramsFromForm(filterForm, FIELDS).toString()}`, 'refund-report.csv');
        } catch (error) {
    ...
    page.querySelector('[data-export-pdf]').addEventListener('click', async (event) => {
        ...
        try {
            await downloadReportFile(`/api/v1/admin/reports/refunds/pdf?${paramsFromForm(filterForm, FIELDS).toString()}`, 'refund-report.pdf');
        } catch (error) {
            showError(error.message);
        } finally {
            button.disabled = false;
